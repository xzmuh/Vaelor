...
export function CharactersPage() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo('.character-stage__background', { opacity: 0, scale: 1.04 }, { opacity: 1, scale: 1, duration: 1.2, ease: 'power2.out' })
      gsap.fromTo('.character-stage__art', { opacity: 0, x: 70, scale: 1.035 }, { opacity: 1, x: 0, scale: 1, duration: 1.05, ease: 'power3.out' })
      gsap.fromTo('.character-copy > *', { opacity: 0, x: -24 }, { opacity: 1, x: 0, duration: .7, stagger: .075, ease: 'power3.out' })
      gsap.fromTo('.element-emblem', { opacity: 0, rotate: -25, scale: .75 }, { opacity: .16, rotate: 0, scale: 1, duration: 1.4, ease: 'power3.out' })
    }, pageRef)
    return () => ctx.revert()
  }, [activeIndex, form])
  ...
  return (
    <main className={`characters-page character--${active.id} form--${form}`} ref={pageRef} style={{ '--character-color': active.color } as React.CSSProperties}>
      <div className="character-stage" key={assetId}>
        ...
        <img className="character-stage__art" src={`/chars/${assetId}-fullbody.webp?v=4`} alt={`Arte completa de ${displayName}`} />
